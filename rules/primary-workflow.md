...
- Use `/sk:preview --explain <topic>` to generate visual explanation with ASCII + Mermaid
- Use `/sk:preview --diagram <topic>` for architecture and data flow diagrams
- Use `/sk:preview --slides <topic>` for step-by-step walkthroughs
- Use `/sk:preview --ascii <topic>` for terminal-friendly output only
- **HTML mode** (add `--html` for self-contained HTML pages, opens directly in browser):
  - `/sk:preview --html --explain <topic>` — publication-quality HTML explanation
  - `/sk:preview --html --diagram <topic>` — interactive HTML diagram with zoom controls
  - `/sk:preview --html --slides <topic>` — magazine-quality slide deck
  - `/sk:preview --html --diff [ref]` — visual diff review
  - `/sk:preview --html --plan-review` — plan vs codebase comparison
