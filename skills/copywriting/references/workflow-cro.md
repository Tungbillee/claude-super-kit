...
1. **Screenshots provided** → Use `sk:ai-multimodal` skill to analyze conversion issues
2. **Videos provided** → Use `sk:ai-multimodal` video-analysis for bottleneck identification
3. **URL provided** → Use `web_fetch` tool to fetch and analyze current issues
4. **Scout codebase** → `/sk:scout ext` (preferred) or `/sk:scout` to find relevant files
5. **Implement** → Use `fullstack-developer` agent to write enhanced copy into code files

## CRO Plan Creation Workflow

Use when creating a structured CRO optimization plan (e.g., via `/sk:plan --hard` with CRO-specific requirements).

### Steps

1. **Analyze** → Gather issues from screenshots/videos/URLs using steps above
2. **Activate** `sk:plan` skill for plan structure
