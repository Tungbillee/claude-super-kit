...
1. **Screenshots provided** → Use `sk:ai-multimodal` skill to analyze and describe issues in detail
2. **Videos provided** → Use `sk:ai-multimodal` video-analysis to extract relevant copy issues
3. **Scout codebase** → `/sk:scout ext` (preferred) or `/sk:scout` to find files needing enhancement
