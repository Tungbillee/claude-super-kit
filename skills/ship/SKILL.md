...
Step 8:  Journal          → Write technical journal via /sk:journal
Step 9:  Docs update      → Update project docs via /sk:docs update (official only)
...
User says `/sk:ship` → run full pipeline → output PR URL.
User says `/sk:ship beta` → ship to dev branch with lighter pipeline.
User says `/sk:ship official` → ship to main with full docs + journal.
