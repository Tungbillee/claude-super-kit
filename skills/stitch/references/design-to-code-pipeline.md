...
# 4. Activate sk:frontend-design to implement React components from the spec
```
...
|-------------|-------|-------------|
| React/Vue/Svelte components | `sk:frontend-design` | Converts Tailwind HTML to framework components |
| Full page with style guide | `sk:ui-ux-pro-max` | Builds complete layouts with design system |
| Design token extraction | `sk:ui-styling` | Extracts shadcn/Tailwind tokens from DESIGN.md |

## Handoff Protocol

1. `sk:stitch` exports artifacts to `./stitch-exports/` (or plan directory)
...
# -> Skip Stitch, use sk:ui-ux-pro-max for text-based design instead
