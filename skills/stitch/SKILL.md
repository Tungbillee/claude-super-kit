...
1. **Check quota** — Run `stitch-quota.ts check`. If exhausted, suggest `sk:ui-ux-pro-max` fallback.
...
6. **Implement** — Hand off exported artifacts to implementation skill:
   - `sk:frontend-design` — React/Vue/Svelte components from Tailwind export
   - `sk:ui-ux-pro-max` — Full page layouts with style guide integration
   - `sk:ui-styling` — Design token extraction from DESIGN.md
...
- **Fallback:** When exhausted, use `sk:ui-ux-pro-max` for text-based design generation
