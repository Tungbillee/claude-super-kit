...
2. Activate `sk:debug` skill (systematic-debugging + root-cause-tracing techniques).
3. Activate `sk:sequential-thinking` skill — form hypotheses through structured reasoning, NOT guessing.
4. Spawn parallel `Explore` subagents to test each hypothesis against codebase evidence.
5. If 2+ hypotheses fail → auto-activate `sk:problem-solving` skill for alternative approaches.
...
5. Run `/sk:journal` to write a concise technical journal entry upon completion
...
- `sk:sequential-thinking` (Step 2) — structured hypothesis formation

**Conditional:**
- `sk:problem-solving` — auto-triggers when 2+ hypotheses fail in Step 2
- `sk:brainstorm` — multiple valid approaches, architecture decision (Deep only)
- `sk:context-engineering` — fixing AI/LLM/agent code
- `sk:project-management` — moderate+ for task hydration/sync-back
