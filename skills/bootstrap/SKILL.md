...
```
/sk:bootstrap <user-requirements>
```
...
**Example:**
```
/sk:bootstrap "Build a SaaS dashboard with auth" --fast
/sk:bootstrap "E-commerce platform with Stripe" --parallel
```
...
- `--full` → `/sk:plan --hard <requirements>` (thorough research + validation)
- `--auto` → `/sk:plan --auto <requirements>` (auto-detect complexity)
- `--fast` → `/sk:plan --fast <requirements>` (skip research)
- `--parallel` → `/sk:plan --parallel <requirements>` (file ownership + dependency graph)
...
- `--full` → `/sk:cook <plan-path>` (interactive review gates)
- `--auto` → `/sk:cook --auto <plan-path>` (skip review gates)
- `--fast` → `/sk:cook --auto <plan-path>` (skip review gates)
- `--parallel` → `/sk:cook --parallel <plan-path>` (multi-agent execution)
...
- Run `/sk:journal` to write a concise technical journal entry upon completion
