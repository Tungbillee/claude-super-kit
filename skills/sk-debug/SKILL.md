...
Visual verification of frontend implementations via Chrome MCP (Claude Chrome Extension) or `sk:chrome-devtools` skill fallback. Detect if frontend-related → check Chrome MCP availability → screenshot + console error check → report. Skip if not frontend.
...
- **Codebase:** `sk:docs-seeker` skill for package/plugin docs; `sk:repomix` skill for codebase summary
- **Scouting:** `/sk:scout` or `/sk:scout ext` for finding relevant files
- **Frontend:** Chrome browser or `sk:chrome-devtools` skill for visual verification (screenshots, console, network)
- **Skills:** Activate `sk:problem-solving` skill when stuck on complex issues
