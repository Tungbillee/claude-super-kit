...
# sk:predict — Multi-Persona Pre-Analysis
...
- Trivial or low-risk changes (use `sk:debug` for bugs, `sk:plan` for already-decided tasks)
...
|---------------|-------|-----|
| Deepen risk scenarios | `sk:scenario` | Feed Risk Summary rows as feature description |
| Create implementation plan | `sk:plan` | Attach Recommendations as constraints to planner |
| High-risk feature implementation | `sk:cook` | Reference CAUTION/STOP items as acceptance gates |
...
```
/sk:predict "Add WebSocket support for real-time notifications"
/sk:predict "Migrate authentication from JWT to session cookies"
/sk:predict "Add multi-tenancy to the database layer"
/sk:predict "Replace REST API with GraphQL" --files src/api/**/*.ts
```
