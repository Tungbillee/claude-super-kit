...
```
/sk:team <template> <context> [flags]
```

**Templates:** `sk:research`, `sk:cook`, `sk:code-review`, `sk:debug`
...
4. Do NOT fall back to subagents. `/sk:team` MUST use Agent Teams or abort.
...
## ON `/sk:team research <topic>` [--researchers N]:

*Wraps /sk:research skill -- scope, gather, analyze, report.*
...
11. **JOURNAL**: Run `/sk:journal` to write a concise technical journal entry upon completion

---

## ON `/sk:team cook <plan-path-or-description>` [--devs N]:

*Wraps /sk:cook skill -- plan, code, test, review, finalize.*
...
7. **DOCS SYNC EVAL** (MANDATORY for cook -- from /sk:cook finalize):
   ```
...
## ON `/sk:team review <scope>` [--reviewers N]:

*Wraps /sk:code-review skill -- scout, review, synthesize with evidence gates.*
...
10. **JOURNAL**: Run `/sk:journal` to write a concise technical journal entry upon completion

---

## ON `/sk:team debug <issue>` [--debuggers N]:

*Wraps /sk:fix skill -- root-cause-first, adversarial hypotheses, disprove to converge.*
