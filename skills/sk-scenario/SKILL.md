...
# sk:scenario — Edge Case & Scenario Explorer
...
| Next Step | Skill | How |
|-----------|-------|-----|
| Generate test cases from scenarios | `sk:test` | Pass scenario table as input context |
| Inform implementation plan risks | `sk:plan` | Paste Critical/High rows into risk assessment |
| Deep persona debate on top risks | `sk:predict` | Feed Critical scenarios as the change proposal |
...
```
/sk:scenario src/api/payment.ts
/sk:scenario "User registration with OAuth providers"
/sk:scenario src/middleware/auth.ts
/sk:scenario "Add multi-tenancy to the database layer"
```
