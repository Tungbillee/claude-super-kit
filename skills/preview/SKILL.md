...
### View Mode
- `/sk:preview <file.md>` - View markdown file in novel-reader UI
- `/sk:preview <directory/>` - Browse directory contents
- `/sk:preview --stop` - Stop running server

### Generation Mode (Markdown)
- `/sk:preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/sk:preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `/sk:preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/sk:preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)
- `/sk:preview --html --explain <topic>` - Self-contained HTML explanation
- `/sk:preview --html --slides <topic>` - Magazine-quality HTML slide deck
- `/sk:preview --html --diagram <topic>` - HTML diagram with zoom controls
- `/sk:preview --html --diff [ref]` - Visual diff review
- `/sk:preview --html --plan-review [plan-file]` - Plan vs codebase comparison
- `/sk:preview --html --recap [timeframe]` - Project context snapshot
...
| Flag without topic | Ask user: "Please provide a topic: `/sk:preview --explain <topic>`" |
...
| Server startup failure | Check if port in use, try `/sk:preview --stop` first |
...
Use `/sk:mermaidjs-v11` skill for Mermaid syntax validation.
...
- For `--slides`: consider invoking `/sk:ui-ux-pro-max` for richer style selection
