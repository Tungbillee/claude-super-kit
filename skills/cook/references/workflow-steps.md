...
- Use `/sk:scout ext` or `scout` agent for codebase search
...
**Fast:**
- Use `/sk:plan --fast` with scout results only
- Minimal planning, focus on action

**Parallel:**
- Use `/sk:plan --parallel` for dependency graph + file ownership matrix
...
- Use `AskUserQuestion` to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run `/sk:plan validate` skill invocation
